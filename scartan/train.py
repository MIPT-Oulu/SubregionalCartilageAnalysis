...
from scartan.datasets import DatasetOAIiMoSagittal2d, sources_from_path
from scartan.models import dict_models
from scartan.metrics import confusion_matrix, dice_score_from_cm
from scartan.preproc import *
from scartan.repro import set_ultimate_seed
from scartan.various import (dict_losses, CheckpointHandler, dict_optimizers,
...
class ModelTrainer:
    def __init__(self, config, fold_idx=None):
        ...
        self.paths_weights_fold['segm'] = \
            Path(config['path_weights'], 'segm', f'fold_{self.fold_idx}')
        self.paths_weights_fold['segm'].mkdir(exist_ok=True)

        self.path_logs_fold = \
            Path(config['path_logs'], f'fold_{self.fold_idx}')
        self.path_logs_fold.mkdir(exist_ok=True)
        ...
        if self.models['segm'].training:
                ...
                for step_idx in range(steps_ds):
                    ...
                    if self.config['debug']:
                        self.tensorboard.add_scalar(
                            f"fold_{self.fold_idx}/batch_loss_segm_train",
                            loss_segm.item(),
                            global_step=steps_ds * epoch_idx + step_idx)
            ...
            with torch.no_grad(), tqdm(**prog_bar_params) as prog_bar:
                for step_idx in range(steps_ds):
                    ...
                    if self.config['debug']:
                        self.tensorboard.add_scalar(
                            f"fold_{self.fold_idx}/batch_loss_segm_val",
                            loss_segm.item(),
                            global_step=steps_ds * epoch_idx + step_idx)
...
@click.option('--dataset', type=click.Choice(['oai_imo', ]), default='oai_imo')
...
def main(**config):
    config['path_data_root'] = Path(config['path_data_root']).resolve()
    ...
    config['path_logs'] = Path(config['path_experiment_root'], 'logs_train')
    config['path_weights'].mkdir(exist_ok=True)
    config['path_logs'].mkdir(exist_ok=True)

    logging_fh = logging.FileHandler(
        Path(config['path_logs'], 'main_{}.log'.format(config['fold_idx'])))
