...
from scartan.datasets import sources_from_path
from scartan.various import numpy_to_nifti, png_to_numpy
from scartan.models import dict_models
from scartan.preproc import *
from scartan.repro import set_ultimate_seed
from scartan.various import CheckpointHandler
...
def predict_folds(config, loader, fold_idcs):
    ...
    for fold_idx in fold_idcs:
        ...
        paths_weights_fold['segm'] = Path(config['path_weights'], 'segm', f'fold_{fold_idx}')
        ...
        with tqdm(total=len(loader), desc=f'Eval, fold {fold_idx}') as prog_bar:
            for i, data_batch in enumerate(loader):
                ...
                for k, data_dict in enumerate(data_dicts):
                    ...
                    fname_base = Path(data_dict['path_rel_image']).stem

                    # Save the predictions
                    dir_predicts = Path(dir_base, 'mask_folds')
                    dir_predicts.mkdir(exist_ok=True)
                    ...
                    tifffile.imsave(str(fname_full), tmp, compress=9)
...
def merge_predictions(config, source, loader, dict_fns,
    ...
    with tqdm(total=len(df_meta), desc='Merge') as prog_bar:
        for i, row in df_meta.iterrows():
            dir_scan_predicts = Path(
                config['path_predicts'],
                row['patient'], row['release'], row['sequence'])
            dir_image_prep = Path(dir_scan_predicts, 'image_prep')
            dir_mask_prep = Path(dir_scan_predicts, 'mask_prep')
            dir_mask_folds = Path(dir_scan_predicts, 'mask_folds')
            dir_mask_foldavg = Path(dir_scan_predicts, 'mask_foldavg')
            dir_vis_foldavg = Path(dir_scan_predicts, 'vis_foldavg')
            ...
            fname_base = Path(row['path_rel_image']).stem

            fnames_pred = Path(dir_mask_folds).glob(f'{fname_base}_fold_*.*')

            # Read the reference data
            image = cv2.imread(
                str(Path(dir_source_root, row['path_rel_image'])),
                cv2.IMREAD_GRAYSCALE)
            image = dict_fns['crop'](image[None, ])[0]
            image = np.squeeze(image)
            if 'path_rel_mask' in row.index:
                ys_true = loader.dataset.read_mask(
                    Path(dir_source_root, row['path_rel_mask']))
            ...
            fname_full = Path(dir_image_prep, f'{fname_base}.png')
            cv2.imwrite(str(fname_full), image)  # image
            ...
            if ys_true is not None:
                ...
                fname_full = Path(dir_mask_prep, f'{fname_base}.png')
                cv2.imwrite(str(fname_full), ys_true_arg_np)  # mask

            fname_meta = Path(config['path_predicts'], 'meta_dynamic.csv')
            if not fname_meta.exists():
                df_meta.to_csv(fname_meta, index=False)  # metainfo

            # Save ensemble prediction, two versions
            fname_full = Path(dir_mask_foldavg, f'{fname_base}.png')
            cv2.imwrite(str(fname_full), ys_pred_arg_np)

            # Save ensemble visualizations
            if save_plots:
                if ys_true is not None:
                    fname_full = Path(dir_vis_foldavg, f"{fname_base}_overlay_mask.png")
                    save_vis_overlay(image=image,
                                     mask=ys_true_arg_np,
                                     num_classes=config['output_channels'],
                                     fname=fname_full)

                fname_full = Path(dir_vis_foldavg, f"{fname_base}_overlay_pred.png")
                save_vis_overlay(image=image,
                                 mask=ys_pred_arg_np,
                                 num_classes=config['output_channels'],
                                 fname=fname_full)
                ...
                if ys_true is not None:
                    fname_full = Path(dir_vis_foldavg, f"{fname_base}_overlay_diff.png")
    ...
    if convert_to_nifti:
        ...
        for gb_name, gb_df in tqdm(
            ...
            dir_scan_predicts = Path(config['path_predicts'],
                                     patient, release, sequence)
            # for result in ("image_prep", "mask_prep",
            #                "mask_folds", "mask_foldavg", "vis_foldavg"):
            for result in ("image_prep", "mask_prep", "mask_foldavg"):
                pattern = Path(dir_scan_predicts, result, '*.png')
                path_nii = Path(dir_scan_predicts, f"{result}.nii")
...
@click.command()
@click.option('--path_data_root', type=click.Path(exists=True), default='../../data')
@click.option('--path_experiment_root', type=click.Path(), default='../../results/temporary')
@click.option('--model_segm')
...
@click.option('--subset',  type=click.Choice(['test', 'all']))
...
def main(**config):
    ...
    if config['dataset'] == 'oai_imo':
        from scartan.datasets import DatasetOAIiMoSagittal2d as DatasetSagittal2d
    elif config['dataset'] == 'oai_prj_22':
        from scartan.datasets import DatasetOAICustomSagittal2d as DatasetSagittal2d
    else:
        raise ValueError(f"Unknown dataset: {config['dataset']}")

    # Configure dataset-dependent transforms
    fn_crop = CenterCrop(height=300, width=300)
    if config['dataset'] in ('oai_imo', 'oai_prj_22'):
    ...
    dataset_test = DatasetSagittal2d(
        ...
        transforms=[
            PercentileClip(cut_min=10, cut_max=99),
            fn_crop,
            fn_norm,
            ToTensor()
        ])
