...
def index_from_path_oai_imo(path_root, force=False):
    ...
    if not fname_meta_dyn.exists() or force:
        ...
        fnames_mask = list(Path(path_root).glob('**/masks/*.png'))
        ...
        df_meta['path_mask'] = [Path(path_root, e)
...
def read_mask(path_file, mask_mode):
    ...
    """
    mask = cv2.imread(str(path_file), cv2.IMREAD_GRAYSCALE)
    ...
    if mask_mode == 'raw':
        return mask
    elif mask_mode == 'background_femoral':
        ret = np.empty((2, *mask.shape), dtype=mask.dtype)
        ret[1, :, :] = np.isin(mask, locations['femoral']).astype(np.uint8)
        ret[0, :, :] = 1 - ret[1, :, :]
        return ret
    elif mask_mode == 'background_unitibial':
        ret = np.empty((2, *mask.shape), dtype=mask.dtype)
        ret[1, :, :] = np.isin(mask, locations['tibial']).astype(np.uint8)
        ret[0, :, :] = 1 - ret[1, :, :]
        return ret
    elif mask_mode == 'background_patellar':
        ret = np.empty((2, *mask.shape), dtype=mask.dtype)
        ret[1, :, :] = np.isin(mask, locations['patellar']).astype(np.uint8)
        ret[0, :, :] = 1 - ret[1, :, :]
        return ret
    elif mask_mode == 'background_unimeniscus':
        ret = np.empty((2, *mask.shape), dtype=mask.dtype)
        ret[1, :, :] = np.isin(mask, locations['menisci']).astype(np.uint8)
        ret[0, :, :] = 1 - ret[1, :, :]
        return ret
