...
@click.argument('path_root_imo', type=click.Path(exists=True))
...
def main(**config):
    ...
    config['path_root_imo'] = config['path_root_imo'].resolve()
    ...
    def worker_xz9(path_root_output, path_stack, margin):
        ...
        path_annot = Path(config['path_root_imo'], patient, prefix_var)
        fnames_annot = list(path_annot.glob('*.mat'))
        if len(fnames_annot) != 1:
            raise ValueError(f'Unexpected annotations for patient: {patient}')
        fname_annot = fnames_annot[0]

        file_mat = io.loadmat(Path(path_annot, fname_annot),
        ...
        for slice_idx in range(num_slices):
            ...
            dir_rel_mask = Path(patient, release, sequence, 'masks')
            dir_abs_image = Path(path_root_output, dir_rel_image)
            dir_abs_mask = Path(path_root_output, dir_rel_mask)
            ...
            path_abs_mask = Path(dir_abs_mask, fname_mask)
            if not config['meta_only']:
                cv2.imwrite(str(path_abs_mask), mask_proc)

            path_rel_image = Path(dir_rel_image, fname_image)
            path_rel_mask = Path(dir_rel_mask, fname_mask)
            ...
            meta['path_rel_mask'].append(str(path_rel_mask))
        return meta
    # -------------------------------------------------------------------------
    ...
    fnames_sas = config['path_root_oai_mri'].glob('**/*.sas7bdat')
