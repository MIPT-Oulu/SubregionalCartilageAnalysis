...
class Crop(object):
    def __init__(self, output_size):  #, keep_size=True, prob=.5):
...
class ContrastNormalization(object):
    def __init__(self, alpha, centered=False, prob=0.5):
        self.alpha = alpha
    ...
    def __call__(self, image, mask=None):
        ...
        if self.centered:
            ...
            if np.issubdtype(image, int):
                midrange = 128
            elif np.issubdtype(image, float):
                midrange = 0.5
            else:
                raise TypeError('Unsupported type')

        if self.state['p'] < self.prob:
            image = self.alpha * (image - midrange) + midrange
        return image, mask
